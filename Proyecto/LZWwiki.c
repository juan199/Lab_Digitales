...
int main()
{
	int i, fd = open("wabba.txt", O_RDONLY);
	...
	printf("encoded size: %zd\n", _len(enc));
 
	byte *dec = lzw_decode(enc);
	printf("decoded size: %zd\n", _len(dec));
	...
	int fw = open("CodedTest.txt", O_WRONLY | O_CREAT );
	write(fw, enc, st.st_size);
	close(fw);
	
	int fwt = open("DecodedTest.txt", O_WRONLY | O_CREAT );
	write(fwt, dec, st.st_size);
	close(fwt);
